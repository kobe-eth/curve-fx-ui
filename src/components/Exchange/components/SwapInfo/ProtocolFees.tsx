...
import { useUsdPrices, useUsdPricesFromSymbol } from "state/trading/hooks";
...
const ProtocolFees: React.FC<ProtocolFeesProps> = ({
  ...
  useEffect(() => {
    ...
    feesPath.forEach((element, index) => {
      const usdPrice = prices.length > 0 ? prices.find(p => p.symbol === route[index]).usdPrice : 0
      switch (element) {
        case JARVIS_SWAP_FEES:
          newJarvisFee = newJarvisFee + (Number(amountIn) * JARVIS_SWAP_FEES * usdPrice)
